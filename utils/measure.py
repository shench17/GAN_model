...
def compute_PSNR(img1, img2, data_range):
    if type(img1) == torch.Tensor:
        MAE_ = compute_MAE(img1, img2)
        return 10 * torch.log10((data_range ** 2) / MAE_).item()
    else:
        MAE_ = compute_MAE(img1, img2)
        return 10 * np.log10((data_range ** 2) / MAE_)


def compute_SSIM(img1, img2, data_range, window_size=11, channel=1, size_average=True):
    img1_cpu = img1.cpu().squeeze().numpy()
    img2_cpu = img2.cpu().squeeze().numpy()
    #print(img1_cpu.shape)
    return ssim(img1_cpu, img2_cpu,win_size= 11)
