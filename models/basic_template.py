...
class TrainTask(object):
    ...
    def set_loader(self):
        ...
        #test_images = [test_dataset[i] for i in range(0, min(300, len(test_dataset)), 150)]
        
        #low_dose = torch.stack([torch.from_numpy(x) for x in test_images if("ARTERIAL" in x)], dim=0).cuda()
        #full_dose = torch.stack([torch.from_numpy(x) for x in test_images if("NATIVE" in x)], dim=0).cuda()
