...
class corediff(TrainTask):
    ...
    def test(self, n_iter):
        ...
        for low_dose, full_dose in tqdm.tqdm(self.test_loader, desc='test'):
            ...
            ssim_score, mae_score, rmse_score = compute_measure(full_dose, gen_full_dose, data_range)
            ssim_score1, mae_score1, rmse_score1 = compute_measure(full_dose, low_dose, data_range)
            #print(len(self.test_loader))
            ssim1 += ssim_score1 / len(self.test_loader)
            mae1 += mae_score1 / len(self.test_loader)
            rmse1 += rmse_score1 / len(self.test_loader)
            ssim += ssim_score / len(self.test_loader)
            mae += mae_score / len(self.test_loader)
            rmse += rmse_score / len(self.test_loader)

        
        #print(' SSIM1:',ssim1, ' MAE1:',mae1, ' RMSE1:',rmse1)
        print('SSIM:',ssim, ' MAE:',mae, ' RMSE:',rmse)
        print("end")
        self.logger.msg([ssim, mae, rmse], n_iter)
        if opt.wandb:
            wandb.log({'epoch': n_iter, 'SSIM1': ssim1, 'MAE1': mae1, 'RMSE1': rmse1})
            wandb.log({'epoch': n_iter, 'SSIM': ssim, 'MAE': mae, 'RMSE': rmse})
